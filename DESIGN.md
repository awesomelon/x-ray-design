...
```
 역할           | 색상                    | Hex       | 용도
 --------------|------------------------|-----------|----------------------------------------
 Selection     | 보라                    | #8b5cf6   | 호버, 선택 — "선택 계열" 전체
 Snap          | 마젠타                  | #ec4899   | 스냅 가이드, 분배 가이드, 거리 라벨 — "정렬 계열"
```
...
```
 ...
 분배 가이드           | 1px dashed #ec4899 + gap 값 라벨
 거리 라벨             | pill bg #ec4899 + 10px white text
```
...
### Inspect 모드 (Alt+Hover)
```
 인터랙션               | 시각 피드백
 ----------------------|--------------------------------------------------
 Alt+호버 (선택 없음)    | 호버 요소에서 4방향 거리 라벨 (마젠타 pill)
 Alt+호버 (선택 있음)    | 선택↔호버 간 거리 라벨 + 연결선 (마젠타 dashed)
 Alt 해제               | 모든 inspect 라벨/연결선 제거
 Alt+mousedown          | inspect 종료, 드래그 진입
```
...
| 2026-03-22 | ~~Grid 오버레이~~ [Removed] → 요소 기반 magnetic 스냅 | UX 불편, 요소 엣지/중심/분배/거리 스냅으로 전환 (Figma 동등 수준) |
| 2026-03-22 | 계층적 요소 스캔 (형제→부모→삼촌) | Figma 패턴 차용, 웹 DOM 구조에 최적화 |
| 2026-03-22 | 실시간 재스캔 (스크롤 변경 시) | 스크롤 중 드래그 시 정확한 스냅 대상 유지 |
